...
VOID LoadFile(SOUND* file)
{
    ...
    file->SamplesPerSec = mmah.mmXWAVHeader.WAVEHeader.ulSamplesPerSec / 2;
...
/***********************************************************************\
 * function WaveAudio
 * opens the sound device, plays the playlist belonging to file
...
VOID WaveAudio(SOUND file)
{
    ...
    mciSendCommand(mop.usDeviceID, MCI_SET,
			MCI_WAIT | MCI_WAVE_SET_SAMPLESPERSEC | MCI_WAVE_SET_BITSPERSAMPLE,
         &wsp, 0);
